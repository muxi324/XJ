package com.wp.controller.system.head;

import com.wp.controller.base.BaseController;
import com.wp.service.system.appuser.AppuserService;
...
public class HeadController extends BaseController {
	...
	public ModelAndView saveSys() throws Exception{
		...
		Tools.writeFile(Const.EMAIL,pd.getString("SMTP")+",wp,"+pd.getString("PORT")+",wp,"+pd.getString("EMAIL")+",wp,"+pd.getString("PAW"));	//写入邮件服务器配置
		Tools.writeFile(Const.SMS1,pd.getString("SMSU1")+",wp,"+pd.getString("SMSPAW1"));	//写入短信1配置
		Tools.writeFile(Const.SMS2,pd.getString("SMSU2")+",wp,"+pd.getString("SMSPAW2"));	//写入短信2配置
		mv.addObject("msg","OK");
	...
	public ModelAndView saveSys2() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.FWATERM,pd.getString("isCheck1")+",wp,"+pd.getString("fcontent")+",wp,"+pd.getString("fontSize")+",wp,"+pd.getString("fontX")+",wp,"+pd.getString("fontY"));	//文字水印配置
		Tools.writeFile(Const.IWATERM,pd.getString("isCheck2")+",wp,"+pd.getString("imgUrl")+",wp,"+pd.getString("imgX")+",wp,"+pd.getString("imgY"));	//图片水印配置
		//Watermark.fushValue();
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
